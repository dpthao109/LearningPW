...
export class CartPage {
  ...
  rowItemLocator = (itemName: string): Locator =>
    this.page.getByRole("row").filter({ hasText: itemName });
  ...
  async verifyItemInCart(itemName: string, numberOfItems?: number) {
    ...
    if (numberOfItems) {
      const quantityLocator =
        this.rowItemLocator(itemName).getByRole("spinbutton");
  ...
  async verifySubTotal(itemName: string, expectedTotal: number) {
    const expected = expectedTotal
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const subTotalLocator =
      this.rowItemLocator(itemName).locator(".product-subtotal");
